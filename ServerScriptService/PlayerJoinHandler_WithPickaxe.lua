-- PlayerJoinHandler.lua (UPDATED WITH PICKAXE)
-- Replace your existing PlayerJoinHandler with this version
...
Players.PlayerAdded:Connect(function(player)
    ...
    player.CharacterAdded:Connect(function(character)
        ...
        if playerData.FirstTime or not playerData.Guild then
            ...
            -- Give pickaxe to returning player
            spawn(function()
                wait(2)
...
GuildSelectionRemote.OnServerEvent:Connect(function(player, action, guildName)
    if action == "SelectGuild" then
        ...
        if PlayerDataModule.SetPlayerGuild(player, guildName) then
            ...
            -- Give starter pickaxe to new guild member
            spawn(function()
                wait(2) -- Wait for player to fully spawn
                MiningModule.CreatePickaxe(player)
                print("Gave starter pickaxe to new guild member: " .. player.Name)
            end)
